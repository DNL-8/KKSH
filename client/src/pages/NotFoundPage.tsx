...
export function NotFoundPage() {
    ...
    return (
        <div
            className={`flex min-h-screen flex-col items-center justify-center px-6 text-center overflow-hidden ${isLightTheme ? "bg-slate-100" : "bg-[#020204]"
                }`}
        >
            {/* Background elements */}
            ...
            {/* Description */}
            <p className={`mb-3 max-w-md text-[13px] font-medium uppercase tracking-wider z-10 ${isLightTheme ? "text-slate-600" : "text-slate-300"
                }`}>
                As coordenadas desta dimensao nao foram encontradas no sistema.
            </p>
            <p className="mb-12 text-[10px] font-bold uppercase tracking-[0.2em] text-[hsl(var(--accent)/0.6)] bg-[hsl(var(--accent)/0.05)] border border-[hsl(var(--accent)/0.1)] px-4 py-2 rounded-lg backdrop-blur-sm z-10 shadow-[0_0_15px_rgba(var(--glow),0.1)]">
                Codigo de Erro: 404 - Rota Inexistente
            </p>
