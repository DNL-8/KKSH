                ...
                <div className="space-y-4 animate-in fade-in duration-500">
                    <div className="flex items-center gap-3 px-1 mb-2">
                        {selectedVideo ? (
                            <>
                                ...
                                <span className="text-sm font-bold text-slate-900 tracking-wide truncate">{selectedVideo.name}</span>
                            </>
                        ) : (
                    ...
                    <div className={`flex flex-col sm:flex-row items-stretch rounded-2xl overflow-hidden divide-y sm:divide-y-0 sm:divide-x divide-white/5 ${isIosTheme ? "ios26-section" : "border border-slate-300/50 liquid-glass/30"}`}>
                        <div className="flex-1 p-5 relative overflow-hidden group">
                            ...
                            <p className="mt-2 text-3xl font-black text-slate-900 relative z-10">{videoCount}</p>
                        </div>
                        <div className="flex-1 p-5 relative overflow-hidden group">
                            ...
                            <p className="mt-2 text-3xl font-black text-slate-900 relative z-10">{folderCount}</p>
                        </div>
                        <div className="flex-[1.5] p-5 relative overflow-hidden group">
                            ...
                            <p className="mt-2 text-base font-bold text-slate-800 truncate relative z-10" title={selectedVideo?.relativePath}>{selectedVideo?.relativePath ?? "-"}</p>
                        </div>
                        <div className={`flex-1 p-5 relative overflow-hidden group transition-colors ${completed ? "bg-emerald-500/5 text-emerald-400" : ""}`}>
                            ...
                            <div className="mt-2 flex items-center gap-2 relative z-10">
                                ...
                                <p className={`text-sm font-black uppercase tracking-wider ${completed ? 'text-emerald-300' : 'text-slate-600'}`}>
                    ...
                    {selectedVideo ? (
                        <div className={`rounded-2xl overflow-hidden ${isIosTheme ? "ios26-section" : "border border-slate-300/50 liquid-glass/30"}`}>
                            ...
                            <div className="p-5 border-b border-slate-300/50 bg-white/[0.02] relative overflow-hidden">
                                ...
                                <p className="text-xl font-black text-slate-900 leading-tight break-words relative z-10">{selectedVideo.name}</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-white/5">
                                <div className="p-5 group hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <p className="font-bold text-slate-800 group-hover:text-slate-900 transition-colors">{formatBytes(selectedVideo.size)}</p>
                                </div>
                                <div className="p-5 group hover:bg-white/[0.02] transition-colors">
                                    <p className="text-[9px] uppercase font-black tracking-widest text-slate-500 mb-1">Data</p>
                                    <p className="font-bold text-slate-800 group-hover:text-slate-900 transition-colors">{formatDate(selectedVideo.createdAt)}</p>
                                </div>
                                <div className="p-5 group hover:bg-white/[0.02] transition-colors">
                            ...
                            <div className="p-5 border-t border-slate-300/50 bg-slate-950/40 flex flex-col sm:flex-row gap-6">
                                <div className="flex-1 min-w-0">
                                    <p className="text-[9px] uppercase font-black tracking-widest text-slate-500 mb-1">Caminho do Diretório</p>
                                    <p className="font-semibold text-slate-800 text-sm truncate">{selectedVideo.relativePath}</p>
                                </div>
