...
def validate_csrf(request: Request) -> None:
    ...
    has_csrf_cookie = bool(request.cookies.get(plain_name) or request.cookies.get(host_name))
    if not has_auth_cookie and not has_csrf_cookie:
        return

    cookie_token = request.cookies.get(host_name) or request.cookies.get(plain_name)
